...
describe("phantom-strike contracts", () => {
  ...
  it("parses a signal decision retrieval payload through the shared registry", () => {
    const payload = {
      signal_id: "df1eab71-aa5f-4ce2-9915-64ccf314e3b9",
      baseline_confidence: 0.62,
      confidence_delta: 0.128,
    ...
    expect(validateContract("signalDecisionRecord", payload)).toEqual(
      SignalDecisionRecordSchema.parse(payload)
    );
  });

  it("keeps the signal decision retrieval response envelope deterministic", () => {
    const envelope = {
      success: true,
      meta: {
        audit: {
    ...
    expect(() => PhantomStrikeResponseSchemas.signalDecisionRecord.parse(envelope)).not.toThrow();
  });
});
